...
const BrokerSettingsForm = dynamic(() =>
  import('@/app/(main)/settings/components/broker/broker-settings-form').then(
    m => m.BrokerSettingsForm
  )
);

const BuyerSettingsForm = dynamic(() =>
  import('@/app/(main)/settings/components/buyer/buyer-settings-form').then(
    m => m.BuyerSettingsForm
  )
);
