...
import { selectPackageDocuments } from '@/redux/features/set-package/selectors/documents.selectors';
import {
  DocumentsTable
} from '@/app/set-package/(pages)/documents/components/documents-list/documents-table';

export const DocumentsList = () => {
  const dataroom = useAppSelector(selectPackageDocuments);
  const hasDocuments = dataroom.length > 0;
  ...
  return (
    <div className='mt-[40px]'>
      ...
      {!hasDocuments ?
        ...
        </div> : <DocumentsTable dataroom={dataroom} />
      }
    </div>
  );
};
