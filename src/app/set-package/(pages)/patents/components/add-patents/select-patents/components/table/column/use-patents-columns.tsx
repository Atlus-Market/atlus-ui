...
import { HeaderCell } from '@/app/set-package/(pages)/patents/components/add-patents/select-patents/components/table/header/header-cell';
import {
  getCheckboxState,
...
import { RowCell } from '@/app/set-package/(pages)/patents/components/add-patents/select-patents/components/table/rows/row-cell';
...
const hasRowBeenEdited = (
  row: Row<PatentTableData>,
  editedPatentsIds: string[]
): boolean => {
    ...
    () => [
      {
        ...
        cell: ({ row, getValue }) => {
          const Checkbox = () => {
            const checkboxState = getCheckboxState<PatentTableData>(
              row,
              rowSelectionState
            );
            return (
              <AtlusCheckbox
                ...
                onChange={e => {
                  ...
                  setRowSelection(
                    getUpdatedSelectedRowsState(row, rowSelectionState)
                  );
                }}
              />
            );
              ...
              return (
                <div className="select-family-cell">
                  <span className="text-dark-grey text-sm font-normal leading-[17px] inline-block">
                    {noPatentsCount} {pluralize('assets', noPatentsCount)} not
                    found in the public patent database
                  </span>
                </div>
              );
            }
              ...
              <div className="select-family-cell">
                ...
                <span className="text-dark-grey text-sm font-normal leading-[17px] inline-block ml-5">
                  <span className="text-soft-black">
                    Select Family ({row.original.familyId})
                  </span>
                  {selectedRowsCount > 0 && (
      ...
      },
      {
        ...
        cell: ({ row, getValue }) => {
          ...
          return (
            <RowCell text={getValue()} className="inline-block !w-[248px]" />
          );
        },
